...
local API_KEY = "c26f8b8ac45c60d276e5e0ef98f47ab3bc5ca6f1"
...
-- Sound IDs
local GUI_APPEAR_SOUND_ID = "rbxassetid://115916891254154"
...
StatusProviderLabel.Position = UDim2.new(0, 0, 0, 35) -- Lowered slightly from 30 to 35
...
CloseButton.Text = "X"
CloseButton.TextColor3 = COLOR_TEXT -- White text
...
local function animateGuiAppearance()
    ...
    local tween = TweenService:Create(StatusFrame, tweenInfo, { 
        Position = UDim2.new(1, -230, 0, 0), -- Final position at top-right corner
...
local function cleanupAll()
    ...
    -- Disconnect all scan button connections and destroy buttons
    ...
    -- Disconnect all result window connections and destroy windows
    ...
    -- Fade out and destroy status GUI
...
-- Query LeakCheck API (Paid version)
local function checkLeakCheck(username)
    local success, response = pcall(function()
        local url = API_URL .. HttpService:UrlEncode(username) .. "?type=username&key=" .. API_KEY
        return game:HttpGet(url)
    end)
    ...
    if not data.success then
        ...
        local errorText = "API Error"
        local showFreeScan = false

        if errorMsg:lower():find("invalid x-api-key") then
            errorText = "API Error (400): Invalid X-API-Key"
        elseif errorMsg:lower():find("invalid type") then
            errorText = "API Error (400): Invalid search type"
        elseif errorMsg:lower():find("invalid email") then
            errorText = "API Error (400): Invalid email format"
        elseif errorMsg:lower():find("invalid query") then
            errorText = "API Error (400): Invalid query"
        elseif errorMsg:lower():find("invalid domain") then
            errorText = "API Error (400): Invalid domain"
        elseif errorMsg:lower():find("too short query") then
            errorText = "API Error (400): Query too short (< 3 characters)"
        elseif errorMsg:lower():find("invalid characters") then
            errorText = "API Error (400): Invalid characters in query"
        elseif errorMsg:lower():find("missing x-api-key") then
            errorText = "API Error (401): Missing X-API-Key header"
        elseif errorMsg:lower():find("active plan required") then
            errorText = "API Error (403): Active plan required"
            showFreeScan = true
        elseif errorMsg:lower():find("limit reached") then
            errorText = "API Error (403): Query limit reached. Limit refreshes daily. Try again tomorrow."
            showFreeScan = true
        elseif errorMsg:lower():find("could not determine search type") then
            errorText = "API Error (422): Could not determine search type automatically"
        elseif errorMsg:lower():find("too many requests") then
            errorText = "API Error (429): Too many requests (rate limit exceeded)"
            showFreeScan = true
        else
    ...
    if found > 0 then
        ...
        for _, breach in ipairs(breaches) do
            ...
            local leakData = { source = source, date = breachDate, fields = {} }
            for _, field in ipairs(fields) do
                if breach[field] then
...
local function showResult(player, result)
    -- If a result window already exists, update it
    if playerResultWindows[player] then
        ...
        -- Clear any direct children of frame (e.g., "No leaks found" text)
        ...
        if resultList then
            if result.status == "leaks" and #leaks > 0 then
                ...
                -- Combine Source and Date into fields list
                local fieldsList = {
                ...
                -- Adjust ResultScroll size based on number of leaks
                ...
                if #fieldsList == 0 then
                    ...
                    for _, entry in ipairs(fieldsList) do
                        -- Field name (e.g., "Source", "Date", "Country")
                        ...
                        -- Field value (e.g., "Fling.com", "2011-03", "US")
                ...
                -- Adjust ResultScroll size for no leaks
                local resultScroll = frame:FindFirstChild("ResultScroll")
                if resultScroll then
        ...
        -- Fade in the window and all buttons
        frame.Visible = true
    ...
    -- Create a scrolling frame to hold the list of buttons
    ...
    -- Store result window, its connections, and pagination state
    ...
    -- Free scan button functionality
    ...
    -- Dragging functionality for ResultFrame
    ...
    -- Update ResultFrame position and visibility based on distance
    ...
    playerResultWindows[player].connection = RunService.Heartbeat:Connect(function()
        if player and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Head") then
            ...
            if localPlayer and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                local frameWidth = 350 -- ResultFrame.Size.X.Offset
                local frameHeight = 300 -- ResultFrame.Size.Y.Offset
...
local function createScanButton(player)
    ...
    -- Store button and its connection
    ...
    local function updateButton()
        ...
        if onScreen and distance <= SCAN_DISTANCE and isScanningEnabled then
            button.Position = UDim2.new(0, screenPos.X - 40, 0, screenPos.Y - 15)
    ...
    -- Connect button update to Heartbeat
...
-- Remove scan button and result window for a player
local function removeScanButton(player)
    if playerScanButtons[player] then
...
-- Initialize scan buttons for existing players
